...
drop policy if exists supervisor_insert_parts on public.parts;
create policy supervisor_insert_parts on public.parts
for insert with check (public.current_employee_role() = 'supervisor');

drop policy if exists supervisor_insert_operators on public.operators;
create policy supervisor_insert_operators on public.operators
for insert with check (public.current_employee_role() = 'supervisor');
